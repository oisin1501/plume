...
async fn fill_missing(column: String, strategy: String, state: State<'_, AppState>) -> Result<DataSummary, String> {
    let mut guard = state.dataframe.lock().unwrap();
    let df = guard.as_ref().ok_or("No dataset loaded")?;

    // Save current state to history
    state.history.lock().unwrap().push(df.clone());

    let new_df = data::fill_missing_values(df, &column, &strategy)?;
    let summary = data::get_summary(&new_df);
    *guard = Some(new_df);
    state.transform_log.lock().unwrap().push(TransformEntry {
...
async fn train_model(
    ...
    // Detect separator from the stored file
    let sample = std::fs::read_to_string(&file_path)
        .map_err(|e| format!("Failed to read file: {}", e))?
        .chars()
        .take(8192)
        .collect::<String>();
    let separator = data::detect_separator_pub(&sample);

    let sep_str = String::from_utf8(vec![separator]).unwrap_or(",".into());
...
async fn export_model_pickle(
    ...
    let file_path = state.file_path.lock().unwrap().clone().ok_or("No file loaded")?;
    let sample = std::fs::read_to_string(&file_path)
        .map_err(|e| format!("Failed to read file: {}", e))?
        .chars().take(8192).collect::<String>();
    let sep = String::from_utf8(vec![data::detect_separator_pub(&sample)]).unwrap_or(",".into());
